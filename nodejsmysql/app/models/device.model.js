...
Device.create = (newDevice, result) => {
    connDB.query("INSERT INTO `demodb`.`Device` SET ?", newDevice, (err, res) => {
        if (err) {
...
Device.getAll = (department, result) => {
    let sql = `SELECT * FROM demodb.Device`;
...
Device.getDevicebyVender = (vender, result) => {
    ...
    if (vender == '0') {
        sql = `SELECT * FROM demodb.Device`;
    } else {
        sql = `SELECT * FROM demodb.Device${vender}`;
    }
...
Device.findById = (id, result) => {
    connDB.query(`SELECT * FROM demodb.Device WHERE id = ${id}`, (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(err, null);
            return;
...
Device.updateById = (id, device, result) => {
    let sql = `PDATE demodb.Device SET Band = ?, Model = ?, DeviceType = ?, 
...
Device.remove = (id, result) => {
    connDB.query("DELETE FROM `demodb`.`Device` WHERE id = ?", id, (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(null, err);
            return;
...
Device.getFilterDevice = (req, result) => {
    ...
    if (req.vender !== null) {
        if(req.vender === 'BPS')
        sql = `SELECT * FROM demodb.Device${req.vender} `;
    } 
    if (req.strDate !== null) {
